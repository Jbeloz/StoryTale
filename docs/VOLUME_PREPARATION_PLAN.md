...
- [x] Generate and approve one locked human identity and reusable rig per
  approved Story Bible roster entry.
- [x] Remove the flat green background locally and split the exact master into
  a head plus nine named body layers.
- [x] Store hierarchy, pivots, neutral offsets, canvas size, layer order,
  compatible poses, and compatible face sets with the human catalog record.
- [x] Reject a split if its locally rejoined neutral image changes the cleaned
  master.
- [x] Register the stable rig, asset IDs, actor profile, face profile,
  appearance lock, and existing voice mapping in the Story Bible.
- [x] Reuse the real book character ID and generated rig in prepared Gemini
  chapter catalogs instead of regenerating or substituting a generic actor.
- [x] Render the generated layers with neutral, talking, pointing, and walking
  pose transforms.
...
binary store. Durable files, restart recovery, and later-volume reuse across
app restarts are Phase 8.
