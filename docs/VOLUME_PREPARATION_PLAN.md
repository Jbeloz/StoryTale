...
- [x] Remove the flat green background locally and split the exact master into
  ten provisional image regions.
- [x] Store hierarchy, pivots, neutral offsets, canvas size, layer order,
  advertised pose IDs, and advertised face-set IDs with the provisional human
  catalog record.
...
- [x] Reuse the real book character ID and provisional generated rig ID in
  prepared Gemini chapter catalogs instead of regenerating or substituting a
  generic actor.
- [x] Render the generated regions with provisional hard-coded neutral,
...
app restarts remain Phase 8 work after the Phase 7 character gate passes.
