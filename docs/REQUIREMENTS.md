...
- Gemini 3.1 Flash Image generates only missing appearance components through
  ...
  images.
- Face details, hair, and fitted clothing come from one versioned separated
  character sheet with the exact canonical size and part arrangement.
  StoryTale removes green, cuts the fixed cells, applies local masks, and
  attaches every accepted layer to its matching rig part.
