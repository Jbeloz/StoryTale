# StoryTale Character Sheet V3 Provider Contract
...
1. `guide.png` - the exact `4096 x 1024` separated-part layout.
...
- Return one PNG at exactly `4096 x 1024` using the `4:1` aspect ratio and `2K`
  provider size.
...
- All four hair layers belong to the same character: identical color, line
  style, texture, highlights, and crown/hairline logic. Only rear length and
  the natural construction required by that length may differ.
- Each rear option must attach correctly to the same front-hair layer and head.
- Hair stays independent from the head and body pixels.
- `none` is a runtime visibility choice and does not require an image cell.
...
- Keep one identity across the face details, hair catalog, palette, and outfit.
...
hair options describe different characters, clothing fails across seams, or
the separated parts cannot reproduce the supplied assembled character.
